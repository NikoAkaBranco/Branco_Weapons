...
Citizen.CreateThread(function()
	while true do
		...
		if weapon == GetHashKey("weapon_pistolxm3") then		
			if IsPedShooting(ped) then
				ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0.015)
			end
		end
		...
		if weapon == GetHashKey("WEAPON_ASSAULTRIFLE") then			
			if IsPedShooting(ped) then
				ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0.058)
			end
		end
		...
		if weapon == GetHashKey("WEAPON_CARBINERIFLE") then	
			if IsPedShooting(ped) then
				ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0.078)
			end
		end
...
Citizen.CreateThread(function()   --- dont edit 
	while true do
		Citizen.Wait(0)
